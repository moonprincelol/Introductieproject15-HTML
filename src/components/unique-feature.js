...
import './unique-feature.css'

const UniqueFeature = (props) => {
  return (
    <div className={`unique-feature-feature-card ${props.rootClassName} `}>
      <div className="unique-feature-container">
        <svg viewBox="0 0 1024 1024" className="unique-feature-icon">
          <path
      ...
      <h2 className="unique-feature-text">{props.title}</h2>
      <span className="unique-feature-text1">{props.description}</span>
    </div>
  )
}

UniqueFeature.defaultProps = {
  title: 'Lorem ipsum',
  rootClassName: '',
  description:
